// OldestArticlePage.js
import React, { useEffect, useState } from 'react';
import axios from 'axios';
import { Link } from 'react-router-dom';

const OldestArticlePage = () => {
    const [articles, setArticles] = useState([]);
    const [nextPage, setNextPage] = useState(null);
    const [previousPage, setPreviousPage] = useState(null);
    const [error, setError] = useState(null);
    const [currentPageUrl, setCurrentPageUrl] = useState('http://localhost:8000/articles/old/?ordering=create_at');
    ...
    useEffect(() => {
        const fetchArticles = async () => {
            try {
                ...
                setArticles(data.results); // 오래된 순서로 받아온 목록
                setNextPage(data.next);
                setPreviousPage(data.previous);
                setTotalPages(Math.ceil(data.count / 5)); // 페이지 수 계산
            } catch (err) {
                setError('오래된 기사를 가져오는 데 실패했습니다.');
            }
        };
    ...
    const renderPaginationButtons = () => {
        ...
        for (let page = 1; page <= totalPages; page++) {
            buttons.push(
                <button
                    key={page}
                    onClick={() => handlePageChange(`http://localhost:8000/articles/old/?ordering=create_at&page=${page}`, page)}
    ...
    return (
        <div className="bg-white text-gray-800">
            <main className="max-w-7xl mx-auto px-4 py-6">
                ...
                {articles.length === 0 && !error && <p>오래된 기사가 없습니다.</p>}
...
export default OldestArticlePage;
